...
import { getAdminSupportTickets, updateSupportTicketStatus } from '@marketplace/shared-hooks';
...
export default function AdminSupportScreen({ navigation }: any) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setTickets(data);
    } catch { Alert.alert('خطأ', 'فشل تحميل التذاكر'); }
  ...
  const handleUpdateStatus = async (ticketId: string, status: string) => {
    ...
    try {
      ...
      load();
    } catch { Alert.alert('خطأ', 'فشل تحديث حالة التذكرة'); }
  ...
  const renderTicket = ({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity style={s.card} onPress={() => setSelected(item)} activeOpacity={0.9}>
          ...
          <View style={s.modalBox}>
            ...
            {selected && (
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{gap: 20}}>
                ...
                <View style={s.detailBlock}>
                  <Text style={s.detailLabel}>نص الرسالة</Text>
                  <View style={s.messageBox}>
                    <Text style={s.detailValueMsg}>{selected.message}</Text>
                  </View>
                </View>
